...
printReturnValue PROC
 ...
 ;
 ; Offset of (only) function parameter
 ; on stack relative to rsp:
 ;
   param         = 288

 ;
 ; Offsets off local variables
 ; relative to rsp on stack:
 ;  
   charsWritten  = 48
   stdOut        = 56
   buf           = 64
...
; 24   : static void printReturnValue(signed int ret) {
 ...
 ;
 ; Make room for local parameters on
 ; stack
 ;
   sub   rsp, 280                                ; 00000 48 81 ec 18 01 00 00    ; 00000118H
 ;
 ; Save (only) parameter on stack:
 ;
   mov   DWORD PTR [rsp+param], ecx              ; 00007 89 4c 24 08  

; 25   : 
; 26   :    char buf[200];
; 27   : 
; 28   :    void* stdOut = GetStdHandle(-11); 
...
; 31   :    wsprintfA(buf, "MessageBox with title %s and text %s returned %d\n", message_title, message_text, ret);
 ...
 ; Call wsprintfA
 ;   5th argument:
   mov   eax, DWORD PTR [rsp+param]              ;  0001a  8b 84 24 20 01 00 00
   mov   DWORD PTR [rsp+32], eax                 ;  00021  89 44 24 20
 ; 
 ;   arguments 4 through 1:
 ;
...
; 33   :    WriteConsoleA(stdOut, buf, lstrlen(buf), &charsWritten, 0);
   ...
   lea   r9 , QWORD PTR [rsp+charsWritten]       ;  00057 4c 8d 4c 24 30            
